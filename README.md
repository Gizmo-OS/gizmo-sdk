...
[![NPM version](<https://img.shields.io/npm/v/gizmo.svg?label=npm%20(stable)>)](https://npmjs.org/package/gizmo) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/gizmo)
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install gizmo`
...
console.log(response.additionalMetadata);
```
...
const params: Gizmo.Singlefamily.Pricing.Loans.AllInPricingCreateWholeLoanPricingParams = {
  Context: {},
  Loans: [
    {
      AmortizationRule: { loanAmortizationPeriodCount: '26912511', loanAmortizationType: 'AdjustableRate' },
...
const response: Gizmo.Singlefamily.Pricing.Loans.AllInPricingCreateWholeLoanPricingResponse =
  await client.singlefamily.pricing.loans.allInPricing.createWholeLoanPricing(params);
```
...
<!-- prettier-ignore -->
```ts
const response = await client.singlefamily.pricing.loans.allInPricing
...
const { data: response, response: raw } = await client.singlefamily.pricing.loans.allInPricing
  .createWholeLoanPricing({
    Context: {},
    Loans: [
      {
...
```ts
client.singlefamily.pricing.loans.allInPricing.createWholeLoanPricing({
  // ...
