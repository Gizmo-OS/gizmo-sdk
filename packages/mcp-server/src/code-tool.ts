...
const prompt = `Runs JavaScript code to interact with the Gizmo API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        GIZMO_API_KEY: readEnv('GIZMO_API_KEY') ?? client.apiKey ?? undefined,
        GIZMO_BASE_URL:
          readEnv('GIZMO_BASE_URL') ?? readEnv('GIZMO_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'gizmo',
