...
import { Gizmo, ClientOptions } from 'gizmo';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Gizmo)`
    : `const run: (${functionSource.client}: Gizmo) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(['client.singlefamily.pricing.loans.allInPricing.createWholeLoanPricing'], {
  threshold: 1,
  shouldSort: true,
});
